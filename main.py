...
def KEMF_LED_Alert():
    """Processes seismic data and sends an email if the quality metric is low."""
    try:
        # 1. Fetch Data
        client = Client("IRIS")
        starttime = UTC.now() - 3600
        
        # Note: If this fails due to network, it will be caught by the 'except' block
        ...
        # MHZ - Average PSD for bad data reference
        psd_bad = np.array([
